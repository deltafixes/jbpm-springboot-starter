...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "deltafixes-business-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name deltafixes-business-application-service apps/deltafixes-business-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app deltafixes-business-application-service:1.0-SNAPSHOT
		call oc expose svc/deltafixes-business-application-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd deltafixes-business-application-service
		cd target
