...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "deltafixes-business-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name deltafixes-business-application-service apps/deltafixes-business-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app deltafixes-business-application-service:1.0-SNAPSHOT
    oc expose svc/deltafixes-business-application-service
else

	echo "Launching the application locally..."
	pattern="deltafixes-business-application-service"
	files=( $pattern )
	cd ${files[0]}
